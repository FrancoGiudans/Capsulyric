...
object BackupCategories {
    ...
    private const val PARSER_SUBGROUP_PREFIX = "parser_pkg_"

    /**
     * Read current parser rules from SharedPreferences and return one SubGroup per app.
     * SubGroup IDs use an escaped package name so dots and underscores remain reversible.
    ...
    fun parserAppSubGroups(context: Context): List<SubGroup> {
        ...
        return try {
            ...
            (0 until array.length()).map { i ->
                ...
                val subId = parserSubGroupId(pkg)
     ...
     * Leaf IDs are generated by [parserSubGroupId]. Legacy IDs are still accepted.
     */
    fun filterParserRulesJson(json: String, selectedLeafIds: Set<String>): String {
        val selectedIds = selectedLeafIds
            .filter { it.startsWith("parser_") && it != "parser_all" }
        if (selectedIds.isEmpty()) return json

        val selectedPkgs = selectedParserPackages(selectedIds.toSet())
    ...
    internal fun parserSubGroupId(packageName: String): String {
        val escaped = packageName
            .replace("_", "_u")
            .replace(".", "_d")
        return PARSER_SUBGROUP_PREFIX + escaped
    }

    internal fun packageNameFromParserSubGroupId(id: String): String? {
        if (id.startsWith(PARSER_SUBGROUP_PREFIX)) {
            val encoded = id.removePrefix(PARSER_SUBGROUP_PREFIX)
            ...
            return decoded.toString().takeIf { it.isNotBlank() }
        }

        // IDs created before the escaped format used dots replaced by underscores.
        return id.removePrefix("parser_")
            .replace('_', '.')
            .takeIf { it.isNotBlank() }
    }

    internal fun selectedParserPackages(selectedLeafIds: Set<String>): Set<String> {
        return selectedLeafIds
            .filter { it.startsWith("parser_") && it != "parser_all" }
            .mapNotNull(::packageNameFromParserSubGroupId)
            .toSet()
    }
