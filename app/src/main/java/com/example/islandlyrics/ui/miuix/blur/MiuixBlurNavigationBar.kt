 ...
 *
 * Upstream source: NavigationBar (miuix-ui/src/commonMain/kotlin/top/yukonga/miuix/kmp/basic/NavigationBar.kt) and miuix-blur
 *
 * Modifications by FrancoGiudans for Capsulyric (IslandLyrics):
 *   - Added blurred background variant (MiuixBlurNavigationBar).
 */
...
/**
 * A [NavigationBar] with blur effect.
 */
@Composable
fun MiuixBlurNavigationBar(
    ...
    val navColor = if (color == Color.Unspecified) MiuixTheme.colorScheme.surface else color
    val shouldUseBlur = blurEnabled && backdrop != null

    DemoBlurredNavigationBar(
        modifier = modifier,
        backdrop = backdrop,
        blurEnabled = shouldUseBlur,
        surfaceColor = navColor
    ) {
        NavigationBar(
            modifier = Modifier,
...
@Composable
private fun DemoBlurredNavigationBar(
    modifier: Modifier = Modifier,
